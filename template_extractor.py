...
def extract_from_reaction(reaction):
    ...
    try:
        ...
        reactant_fragments, intra_only, dimer_only = get_fragments_for_changed_atoms(reactants, changed_atom_tags, 
            radius = 2, expansion = [], category = 'reactant') # if category == 'reactants', we include special groups in reaction center.
    ...
    # This following line was commented out by UT
    # rxn_canonical = canonicalize_transform(rxn_string)

    # Change from inter-molecular to intra-molecular 

    # The following three lines were commented out by UT
    ...
    reactants_string = rxn_string.split('>>')[0]
    products_string  = rxn_string.split('>>')[1]
