...
def extract_candidate_reactants_from_cpds(extracted_data, cpds_columnname, smallest_reactant_with_rc, only_rc=False, only_similarity=False, rc_and_similarity=True, similarity_threshold=0.4, match_rc_num_in_reactant=True, narrow_down_database=True):
    ...
    rc = reaction center
    rr = replaced reactant
    match_num_of_rc_in_reactant argument is True, the number of reaction centers in the reactant obtained by retro synthesis and the number of reaction centers in the replaced reactant must be the same.
    ...
    for reactant_with_rc in smallest_reactant_with_rc:
        ...
        for reactant, reaction_centers in reactant_with_rc.items():
            ...
            if (only_rc) and (not match_rc_num_in_reactant):
                replaced_reactants = [cpd for cpd, cpd_mol in zip(cpds, mols) if all(cpd_mol.HasSubstructMatch(rc_mol) for rc_mol in rc_mols)]
            if (only_rc) and (match_rc_num_in_reactant):
                if (len(rc_mols) == 1) or (len(duplicated_rc_dict) == 0):
                    ...
                    replaced_reactants = [cpd for cpd, cpd_mol in zip(cpds, mols) if (all(cpd_mol.HasSubstructMatch(rc_mol) for rc_mol in rc_mols)) and (duplicated_rc_dict[duplicated_rc] == len(cpd_mol.GetSubstructMatches(Chem.MolFromSmarts(duplicated_rc))))]
            ...
            if (only_similarity) and (not match_rc_num_in_reactant):
                ...
                replaced_reactants = [cpd for cpd, cpd_mol in zip(cpds, mols) if DataStructs.TanimotoSimilarity(reactant_ecfp, AllChem.GetMorganFingerprintAsBitVect(cpd_mol, 2, 8192)) >= similarity_threshold]
            if (only_similarity) and (match_rc_num_in_reactant):
                if (len(rc_mols) == 1) or (len(duplicated_rc_dict) == 0):
                    ...
                    duplicated_rc      = list(duplicated_rc_dict.keys())[0]
                    reactant_ecfp      = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(reactant), 2, 8192)
                    replaced_reactants = [cpd for cpd, cpd_mol in zip(cpds, mols) if (DataStructs.TanimotoSimilarity(reactant_ecfp, AllChem.GetMorganFingerprintAsBitVect(cpd_mol, 2, 8192)) >= similarity_threshold) and (duplicated_rc_dict[duplicated_rc] == len(cpd_mol.GetSubstructMatches(Chem.MolFromSmarts(duplicated_rc))))]
            
            if (rc_and_similarity) and (not match_rc_num_in_reactant):
                reactant_ecfp = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(reactant), 2, 8192)
                replaced_reactants = [cpd for cpd, cpd_mol in zip(cpds, mols) if (all(cpd_mol.HasSubstructMatch(rc_mol) for rc_mol in rc_mols)) and (DataStructs.TanimotoSimilarity(reactant_ecfp, AllChem.GetMorganFingerprintAsBitVect(cpd_mol, 2, 8192)) >= similarity_threshold)]
            if (rc_and_similarity) and (match_rc_num_in_reactant):
                if (len(rc_mols) == 1) or (len(duplicated_rc_dict) == 0):
                    ...
                    duplicated_rc      = list(duplicated_rc_dict.keys())[0]
                    reactant_ecfp      = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(reactant), 2, 8192)
                    replaced_reactants = [cpd for cpd, cpd_mol in zip(cpds, mols) if (all(cpd_mol.HasSubstructMatch(rc_mol) for rc_mol in rc_mols)) and (DataStructs.TanimotoSimilarity(reactant_ecfp, AllChem.GetMorganFingerprintAsBitVect(cpd_mol, 2, 8192)) >= similarity_threshold) and (duplicated_rc_dict[duplicated_rc] == len(cpd_mol.GetSubstructMatches(Chem.MolFromSmarts(duplicated_rc))))]

            temp_replaced_reactants.append(replaced_reactants)
        reactant_with_rr = dict(zip(temp_reactant, temp_replaced_reactants))
        result.append(reactant_with_rr)
...
def extract_candidate_reactants_from_mols(extracted_data, cpds_columnname, ecfp_columnname, mols_columnname, smallest_reactant_with_rc, only_rc=False, only_similarity=False, rc_and_similarity=True, similarity_threshold=0.4, match_rc_num_in_reactant=True, narrow_down_database=True):
    # Narrow down the zinc database to be searched by the number of heavy atoms of the predicted reactants.
    '''This function retrieves compounds with all reaction center of the reactant under consideration.
    rc = reaction center
    rr = replaced reactant
    match_num_of_rc_in_reactant argument is True, the number of reaction centers in the reactant obtained by retro synthesis and the number of reaction centers in the replaced reactant must be the same.
    ...
    for reactant_with_rc in smallest_reactant_with_rc:
        ...
        for reactant, reaction_centers in reactant_with_rc.items():
            ...
            # Obtain compounds with all reaction centers of the reactant under consideration.
            if (only_rc) and (not match_rc_num_in_reactant):
                replaced_reactants = [cpd for cpd, cpd_mol in zip(zinc_cpds, zinc_mols) if all(cpd_mol.HasSubstructMatch(rc_mol) for rc_mol in rc_mols)]
            if (only_rc) and (match_rc_num_in_reactant):
                if (len(rc_mols) == 1) or (len(duplicated_rc_dict) == 0):
                    replaced_reactants = [cpd for cpd, cpd_mol in zip(zinc_cpds, zinc_mols) if all(cpd_mol.HasSubstructMatch(rc_mol) for rc_mol in rc_mols)]
                elif len(duplicated_rc_dict) == 1:
                    duplicated_rc      = list(duplicated_rc_dict.keys())[0]
                    replaced_reactants = [cpd for cpd, cpd_mol in zip(zinc_cpds, zinc_mols) if (all(cpd_mol.HasSubstructMatch(rc_mol) for rc_mol in rc_mols)) and (duplicated_rc_dict[duplicated_rc] == len(cpd_mol.GetSubstructMatches(Chem.MolFromSmarts(duplicated_rc))))]
            
            if (only_similarity) and (not match_rc_num_in_reactant):
                reactant_ecfp      = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(reactant), 2, 8192)
                replaced_reactants = [cpd for cpd, cpd_ecfp in zip(zinc_cpds, zinc_ecfps) if DataStructs.TanimotoSimilarity(reactant_ecfp, cpd_ecfp) >= similarity_threshold]
            if (only_similarity) and (match_rc_num_in_reactant):
                if (len(rc_mols) == 1) or (len(duplicated_rc_dict) == 0):
                    reactant_ecfp      = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(reactant), 2, 8192)
                    replaced_reactants = [cpd for cpd, cpd_ecfp in zip(zinc_cpds, zinc_ecfps) if DataStructs.TanimotoSimilarity(reactant_ecfp, cpd_ecfp) >= similarity_threshold]
                    ...
                    duplicated_rc      = list(duplicated_rc_dict.keys())[0]
                    reactant_ecfp      = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(reactant), 2, 8192)
                    replaced_reactants = [cpd for cpd, cpd_mol, cpd_ecfp in zip(zinc_cpds, zinc_mols, zinc_ecfps) if (DataStructs.TanimotoSimilarity(reactant_ecfp, cpd_ecfp) >= similarity_threshold) and (duplicated_rc_dict[duplicated_rc] == len(cpd_mol.GetSubstructMatches(Chem.MolFromSmarts(duplicated_rc))))]
            
            if (rc_and_similarity) and (not match_rc_num_in_reactant):
                reactant_ecfp = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(reactant), 2, 8192)
                replaced_reactants = [cpd for cpd, cpd_mol, cpd_ecfp in zip(zinc_cpds, zinc_mols, zinc_ecfps) if (all(cpd_mol.HasSubstructMatch(rc_mol) for rc_mol in rc_mols)) and (DataStructs.TanimotoSimilarity(reactant_ecfp, cpd_ecfp) >= similarity_threshold)]
            if (rc_and_similarity) and (match_rc_num_in_reactant):
                if (len(rc_mols) == 1) or (len(duplicated_rc_dict) == 0):
                    reactant_ecfp = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(reactant), 2, 8192)
                    replaced_reactants = [cpd for i, (cpd, cpd_mol, cpd_ecfp) in enumerate(zip(zinc_cpds, zinc_mols, zinc_ecfps)) if (all(cpd_mol.HasSubstructMatch(rc_mol) for rc_mol in rc_mols)) and (DataStructs.TanimotoSimilarity(reactant_ecfp, cpd_ecfp) >= similarity_threshold)]
                elif len(duplicated_rc_dict) == 1:
                    duplicated_rc      = list(duplicated_rc_dict.keys())[0]
                    reactant_ecfp      = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(reactant), 2, 8192)
                    replaced_reactants = [cpd for i, (cpd, cpd_mol, cpd_ecfp) in enumerate(zip(zinc_cpds, zinc_mols, zinc_ecfps)) if (all(cpd_mol.HasSubstructMatch(rc_mol) for rc_mol in rc_mols)) and (DataStructs.TanimotoSimilarity(reactant_ecfp, cpd_ecfp) >= similarity_threshold) and (duplicated_rc_dict[duplicated_rc] == len(cpd_mol.GetSubstructMatches(Chem.MolFromSmarts(duplicated_rc))))]
...
def caluculate_AAscore(input_cpd, pretrain_path, extracted_data, used_reactants_num=7):

    multitask = MultiTaskModel(pretrain_path=pretrain_path)
    ...
    if len(round_trip_reactants) == 0:
        print('Since there were no predicted reactants after curation, the score cannot be calculated.')
        return

    retro_time = time.time() - retro_start
    print('retro time: ', retro_time)
    ...
    # extracted_data = pd.read_table(zinc_path, usecols=['washed_isomeric_aromatic_smiles'])
    
    extract_start  = time.time()
    
    reactant_with_rr = extract_replaced_reactants_ver4(extracted_data,
                                                       cpds_columnname='washed_isomeric_aromatic_smiles',
                                                       ecfp_columnname='ECFP4',
                                                       mols_columnname='mols',
                                                       smallest_reactant_with_rc=curated_smallest_reactant_with_rc,
                                                       only_rc=False, only_similarity=False, rc_and_similarity=True, similarity_threshold=0.4, match_rc_num_in_reactant=True)

    extract_time = time.time() - extract_start
    print('extract time: ', extract_time)
    reactant_with_rr = [remove_original_reactant_from_reac_with_rr(reac_with_rr) for reac_with_rr in reactant_with_rr]
    ...
    reactant_with_rr = [get_element_with_max_length_in_values(reac_with_rr) for reac_with_rr in reactant_with_rr]
    vr_combinations  = replace_original_reac_to_virtual_reac(selected_round_trip_reactants, reactant_with_rr)
    ...
    print('Replaced reactants: ', flattened_vr)
    print('Predicted products: ', flattened_predicted_products)

    forward_prediction_time = time.time() - forward_start
    print('forward prediction time: ', forward_prediction_time)
    ...
    predicted_analogs_num = len(set(predicted_analogs))
...
if __name__ == '__main__':
    if 1:
        ...
        pretrain_path   = 'model/2type_mixed_completely_curated/'
        # zinc_tsv_path   = 'data/curated_zinc15/curated_zinc15_with_num_rings.tsv'
        # zinc_sdf_path   = 'data/curated_zinc15/curated_zinc15.sdf'
        zinc_pkl_path   = 'data/curated_zinc15/curated_zinc15_with_mols_and_ecfp.pkl'
        extracted_data  = pd.read_pickle(zinc_pkl_path)
        caluculate_AAscore(input_cpd, pretrain_path, extracted_data)

    if 0:
        zinc_pkl_path = 'data/curated_zinc15/curated_zinc15_with_mols.pkl'
        # sample_path = 'data/curated_zinc15/curated_zinc15_sample.pkl'
        zinc_df = pd.read_pickle(zinc_pkl_path)
        # sample_df = pd.read_pickle(sample_path)
        # zinc_df['mols'] = [Chem.MolFromSmiles(cpd) for cpd in zinc_df['washed_isomeric_aromatic_smiles']]
        # zinc_df['heavy_atoms_num'] = [cpd.GetNumHeavyAtoms() for cpd in zinc_df['mols']]
        zinc_df['ECFP4'] = [AllChem.GetMorganFingerprintAsBitVect(cpd, 2, 8192) for cpd in zinc_df['mols']]
        zinc_df = zinc_df[['washed_isomeric_aromatic_smiles', 'heavy_atoms_num', 'ECFP4', 'mols']]
        # sample_df.to_pickle('data/curated_zinc15/curated_zinc15_sample.pkl')
        zinc_df.to_pickle('data/curated_zinc15/curated_zinc15_with_mols_and_ecfp.pkl')
        print(3)

    if 0:
        target_rc = '[NH2;D1;+0:1]-[c:2](:[c:3]):[c:4]'
        rc_mol = Chem.MolFromSmarts(target_rc)
        zinc_sdf_path   = 'data/curated_zinc15/curated_zinc15.sdf'
        zinc_path       = 'data/curated_zinc15/curated_zinc15_with_num_rings.tsv'
        cpds_columnname='washed_isomeric_aromatic_smiles'
        # zinc_mols = Chem.ForwardSDMolSupplier(zinc_sdf_path)
        # zinc_df = PandasTools.LoadSDF(zinc_sdf_path)
        extracted_data = pd.read_table(zinc_path, usecols=['washed_isomeric_aromatic_smiles'])
        zinc_mols = Chem.SDMolSupplier(zinc_sdf_path)
        for i, zinc_mol in enumerate(zinc_mols):
            if zinc_mol.HasSubstructMatch(rc_mol):
                print(i, Chem.MolToSmiles(zinc_mol))
        start = time.time()
        zinc_mols_list = [zinc_mol for i, zinc_mol in enumerate(zinc_mols)]
        print(time.time() - start)
        for i, (zinc_mol, cpd) in enumerate(zip(zinc_mols, extracted_data[cpds_columnname])):
            print(Chem.MolToSmiles(zinc_mol, kekuleSmiles=True))
            if i == 10:
                break
        zinc_cpds = [extracted_data[cpds_columnname][i] for i, zinc_mol in enumerate(zinc_mols) if (zinc_mol is not None) and (i < 10)]
        print(3)

    if 0:
        smallest_reactant = '[NH2:13][c:14]1[cH:15][cH:16][cH:17][cH:18][cH:19]1'
        smallest_reactant_ecfp = AllChem.GetMorganFingerprintAsBitVect(Chem.MolFromSmiles(smallest_reactant), 2, 8192)
        zinc_path       = 'data/curated_zinc15/curated_zinc15_with_num_rings.tsv'
        zinc_sdf_path   = 'data/curated_zinc15/curated_zinc15.sdf'
        cpds_columnname='washed_isomeric_aromatic_smiles'
        extracted_data  = pd.read_table(zinc_path, usecols=['washed_isomeric_aromatic_smiles'])
        zinc_mols = Chem.SDMolSupplier(zinc_sdf_path)
        replaced_reactants = [cpd for i, (cpd, cpd_mol) in enumerate(zip(extracted_data[cpds_columnname], zinc_mols)) if (DataStructs.TanimotoSimilarity(smallest_reactant_ecfp, AllChem.GetMorganFingerprintAsBitVect(cpd_mol, 2, 8192)) >= 0.4)]
        print(3)
